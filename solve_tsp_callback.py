...
def solve_tsp_callback(node, vehicle, travel):
	...
	m.modelSense = GRB.MINIMIZE  # 最小化目标函数

	m.Params.timeLimit = 600  # 设置求解时间限制
	...
	m.Params.lazyConstraints = 1  # 使用延迟约束
	m.optimize(subtourelim)  # 优化模型
	...
	myTour.append(c+1)  # 添加终点节点,得到完整的TSP路线
	
	# Build the assignment
	vehicleType = TYPE_TRUCK  # 车辆类型
	UAVsOnBoard = []  # 无人机数量
	startAltMeters = 0.0  # 起点高度
	endAltMeters = 0.0  # 终点高度

	i = 0					# 从起点开始
	mayEnd = 0
	tmpDepart = 0.0
	icon = 'ub_truck_1.gltf'  # 车辆图标

	for myIndex in range(1,len(myTour)):
		j = myTour[myIndex]
		# We are traveling from i to j
		# Capture the "traveling" component:
		statusID 	= TRAVEL_TRUCK_EMPTY   # 卡车空载行驶
		ganttStatus	= GANTT_TRAVEL  # 甘特图状态
		startTime 	= tmpDepart	# When we departed from i  # 出发时间（继承上一任务结束时间）
		startNodeID = i  # 起点节点ID
		startLatDeg = node[i].latDeg  # 起点纬度
		startLonDeg	= node[i].lonDeg  # 起点经度
		endTime		= startTime + tau[i][j]	# This is when we arrive at j  # 到达时间
		endNodeID	= j  # 终点节点ID
		endLatDeg	= node[j].latDeg  # 终点纬度
		endLonDeg	= node[j].lonDeg  # 终点经度
		if ((i in C) and (j in C)):
			description 	= 'Driving from Customer %d to Customer %d' % (i,j)  # 卡车从客户i到客户j
		elif ((i == 0) and (j in C)):
			description 	= 'Driving from Depot to Customer %d' % (j)  # 卡车从起点到客户j
		elif ((i in C) and (j == c+1)):
			description 	= 'Returning to the Depot from Customer %d' % (i)  # 卡车从客户i返回起点
		elif ((i == 0) and (j == c+1)):
			description 	= 'Truck 1 was not used'  # 卡车1未使用
		else:
		...
		if (j == c+1):
			...
			pkgIcon 		= packageIcons[1]  # 不同包裹图标，用不同颜色图表表示
		...
		tmpDepart = endTime  # 更新出发时间
		if (j != c+1):  # 如果不是终点
			# Go to the next arc
			i = j

	return (objVal, assignments, packages, myTour)  # 返回目标值、分配、包裹和路线