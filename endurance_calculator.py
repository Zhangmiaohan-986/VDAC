...
# 根据给定 sortie (v,i,j,k) 和特定速度 (speedij, speedjk) 下的最大可能持续时间 (秒)。
...
def give_endurance(node, vehicle, travel, v, i, j, k, totalTimeij, totalTimejk, speedij, speedjk):
	...
	totalTimeij = travel[v][i][j].totalTime  # 将秒转换为小时
	totalTimejk = travel[v][j][k].totalTime  # 将秒转换为小时
	TTvij 	= travel[v][i][j].takeoffTime  # 将秒转换为小时
	...
	TTvjk 	= travel[v][j][k].takeoffTime  # 将秒转换为小时
	FTvjk 	= totalTimejk - travel[v][j][k].takeoffTime - travel[v][j][k].landTime
	LTvjk 	= travel[v][j][k].landTime
	sj_index = node[j].map_key  # 获取客户点j的索引
	sj = node[sj_index].serviceTimeUAV/60  # 处理空中节点对应下的客户点服务时间
	...
	if energy_left >= 0:
		...
		# float(energy_left)/PHover 表示剩余电量可支持的额外悬停时间
